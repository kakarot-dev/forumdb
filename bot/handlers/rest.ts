...
export class DiscordAPI {
  ...
  constructor(client: ForumClient) {
    this.client = client;
    this.rest_api = new REST({ version: "10" });
    this.rest_api.setToken(Bun.env.DISCORD_BOT_TOKEN!);
    this.logger = logger;
  }
    ...
    try {
      ...
      if (!this.client.user?.id)
        return this.logger.error(
          "Client was not resolved while registering commands",
        );

      const commands = (await this.rest_api.get(
  ...
  public async register(): Promise<void> {
    try {
      ...
      if (!args.includes("--register-cmd")) return;
      if (!this.client.user?.id)
        return this.logger.error(
          "Client was not resolved while registering commands",
        );
      ...
      return this.logger.info(
        `Successfully registered ${this.client.commands.all().size} commands`,
      );
    } catch (err: any) {
  ...
  public async sync(name: string): Promise<void> {
    try {
      ...
      if (!this.client.user?.id)
        return this.logger.error(
          `Client was not resolved while syncing command: ${name}`,
        );
      ...
      this.logger.info(`Successfully synced command: ${name}`);
    } catch (err: any) {
